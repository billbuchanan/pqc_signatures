...
/**
 * \fn int8_t rbc_43_qpoly_annulator(rbc_43_qpoly p, const rbc_43_vec v, uint32_t size)
...
int8_t rbc_43_qpoly_annihilator(rbc_43_qpoly p, const rbc_43_vec v, int32_t size) {
  ...
  int i;
  for(i = 0; i < size; ++i) {
    rbc_43_qpoly_evaluate(p_at_vi, p, v[i]);   // p_i(v_i)
    rbc_43_qpoly_qexp(t, p);                   // (p_i^q)(x)
    rbc_43_qpoly_scalar_mul(p, p, p_at_vi);    // (p_i(v_i) * p_i)(x)
    rbc_43_qpoly_add(p, p, t);                 // (p_i^q + p_i(v_i) * p_i)(x)
  }
...
int8_t rbc_43_qpoly_qexp(rbc_43_qpoly o, const rbc_43_qpoly p) {
  ...
  for(int32_t i = 0 ; i <= p->degree ; ++i) {
    ...
    rbc_43_elt_set(o->values[(i + 1) % RBC_43_FIELD_M], tmp);
  }
  ...
  rbc_43_qpoly_update_degree(o, p->degree + 1);
...
int8_t rbc_43_qpoly_add(rbc_43_qpoly o, const rbc_43_qpoly p1, const rbc_43_qpoly p2) {
  ...
  rbc_43_qpoly_update_degree(o, o->degree);
