...
void gf16mat_linearcomb_half_avx2(unsigned char *M, const unsigned char *F, unsigned F_rows, unsigned F_cols, const unsigned char *lambda, unsigned batch) {
...
#undef MAX_H
    ...
    for (unsigned i = 0; i < F_rows; i++) {
        for (unsigned j = 0; j < F_cols; j++) { 
            ...
            unsigned char d = 0;
            _gf16v_mul_add_avx2(&d, fptr, lambda, batch/2); 
            mat[(i * F_cols) + j] = (d >> 4) ^ (d & 0xf);            
        }
    }
    ...
    for (unsigned i = 0; i < F_rows; i+=2) {
        for (unsigned j = 0; j < F_cols; j++) { 
            ...
            M[i + j * row_bytes ] = (dl & 0x0f) | (dh << 4);
        }
    }
}


void gf256mat_linearcomb_half_avx2(unsigned char *M, const unsigned char *F, unsigned F_rows, unsigned F_cols, const unsigned char *lambda_tabs, unsigned batch) {
    for (unsigned i = 0; i < F_rows; i++) {
        for (unsigned j = 0; j < F_cols; j++) { 
            ...
            // m1 = batch/2
            _gf256v_mul_add_avx2(mptr, fptr, lambda_tabs, batch>>1); 
        }
    }
}
