...
void sign_mira_256_tree_expand(sign_mira_256_seed_tree_t tree, const uint8_t *salt) {
  ...
  for(size_t i = 3; i < (SIGN_MIRA_256_PARAM_N_MPC - 1); i+=4) {
    size_t from = i;
    size_t to = i * 2 + 1;
    ...
    hash_sha3_x4_absorb(&ctx_x4, salt_x4, 2 * SIGN_MIRA_256_SECURITY_BYTES);
    hash_sha3_x4_absorb(&ctx_x4, from_x4, sizeof(uint8_t));
    hash_sha3_x4_absorb(&ctx_x4, tree_from_x4, SIGN_MIRA_256_SECURITY_BYTES);
...
void sign_mira_256_tree_expand_partial(sign_mira_256_seed_tree_t partial_tree, const sign_mira_256_seed_tree_node_t partial_tree_seeds[SIGN_MIRA_256_PARAM_N_MPC_LOG2],
  ...
  uint8_t discard_buffer[2 * SIGN_MIRA_256_SECURITY_BYTES];
