...
void gfqm_mul(gfqm o, const gfqm a, const gfqm b) {
	gf16 product[37];
	gfqm tmp;
	for(int i=1 ; i<37 ; i++) product[i] = 0;
	...
	//Red
	for(int i=36 ; i>=19 ; i--) {
		product[i - 19] ^= product[i];
		product[i - 18] ^= product[i];
		product[i - 17] ^= product[i];
		product[i - 14] ^= product[i];
		//product[i] = 0;
	}

	for(int i=0 ; i<19 ; i++) o[i] = product[i];
}
...
void gfqm_vec_to_string(uint8_t *o, const gfqm_vec e, int size) {
	int bytes = (19 * size + 1) / 2;

	for(int i=0 ; i<19*size/2 ; i++) {
		o[i] = (e[(2*i)/19][(2*i)%19] & 0xf) | (e[(2*i+1)/19][(2*i+1)%19] << 4);
	}

	if(19 * size % 2 == 1) o[bytes - 1] = e[size-1][18] & 0xf;
}
...
void gfqm_vec_from_string(gfqm_vec o, int size, const uint8_t *src) {
	...
	for(int i=0 ; i<19*size/2 ; i++) {
		o[(2*i)/19][(2*i)%19] = src[i] & 0xf;
		o[(2*i+1)/19][(2*i+1)%19] = src[i] >> 4;
	}

	if(19 * size % 2 == 1) o[size-1][18] = src[bytes-1] & 0xf;
}
