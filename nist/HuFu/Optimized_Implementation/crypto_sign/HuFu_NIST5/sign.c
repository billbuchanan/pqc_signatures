...
int crypto_sign_keypair(
	...
	mat_mul_unpack(B, A_hat,A_hat+PARAM_M * PARAM_N*2, R + PARAM_M * PARAM_M, PARAM_M, PARAM_N,PARAM_M);
...
int crypto_sign(
	...
	{    
		...
		for (int i = 0; i < PARAM_M / 4;i++){ // recover B and compute B * signature2
			...
			unpack_new((uint32_t *)realA, 4 * PARAM_N, A_hat+8 * PARAM_N*i,A_hat+2*PARAM_N * PARAM_M+PARAM_N*i/2 , 17*4*PARAM_N/8);
			mat_mul(temp+4*i, realA, p + PARAM_M, 4, PARAM_N,1);
		}
		mat_add(v, v, temp, PARAM_M, 1);

		for (int i = 0; i < PARAM_M / 4; i++){ // unpack B and compute B * p2
			int32_t B[4 * PARAM_M];
			unpack_new((uint32_t *)B, 4 * PARAM_M, pk + PARAM_SEED_BYTES+8 * PARAM_M*i,pk + PARAM_SEED_BYTES+2*PARAM_M * PARAM_M+PARAM_M*i/2 , 17*4*PARAM_M/8);
			mat_mul(temp+4*i, B, p + PARAM_M + PARAM_N, 4, PARAM_M,1);
		}
		...
		for (int i = 0; i < PARAM_N/4;i++){
			int32_t S[4*PARAM_M];
			unpack_mat_r(S, sk + CRYPTO_PUBLICKEYBYTES +2 * PARAM_M * PARAM_M / 8+PARAM_M*i, PARAM_M);
			mat_mul(signature+4*i,S, z, 4, PARAM_M,1);
		...
		for (int i = 0; i < PARAM_M / 4;i++){
			int32_t E[4*PARAM_M];
			unpack_mat_r(E, sk + CRYPTO_PUBLICKEYBYTES+PARAM_M*i, PARAM_M);
			mat_mul(signature0+4*i, E, z, 4, PARAM_M,1);
		}
	...
	*mlen = smlen - sig_len - PARAM_SALT_BYTES - 2;
	...
	if(decompress_result){
		...
		for (int i = 0; i < PARAM_M / 4;i++){ // recover B and compute B * signature2
			int32_t realA[4 * PARAM_N];
			unpack_new((uint32_t *)realA, 4 * PARAM_N, A_hat+8 * PARAM_N*i,A_hat+2*PARAM_N * PARAM_M+PARAM_N*i/2 , 17*4*PARAM_N/8);
			mat_mul(temp+4*i, realA, signature, 4, PARAM_N,1);
		}
		...
		for (int i = 0; i < PARAM_M / 4;i++){ // recover B and compute B * signature2
			int32_t B[4 * PARAM_M];
			unpack_new((uint32_t *)B, 4 * PARAM_M, pk + PARAM_SEED_BYTES+8 * PARAM_M*i,pk + PARAM_SEED_BYTES+2*PARAM_M * PARAM_M+PARAM_M*i/2 , 17*4*PARAM_M/8);
			for(int i = 0; i < 4 * PARAM_M; i++) if(B[i] == 3) B[i] = -1;	
			mat_mul(temp + 4 * i, B, signature + PARAM_N, 4, PARAM_M,1);
		}
