...
int crypto_sign_keypair(unsigned char *pk, unsigned char *sk)
{
    ...
    randombytes1(seed_PK, VOX_SEED_BYTES);
    randombytes1(seed_SK, VOX_SEED_BYTES);
                     ...
                     const unsigned char *pk)
{
    ...
    for (i=0; i<VOX_O; i++) {
        if (i < VOX_T)
            VOX_GenPK_oil(Pub_i, pk, i);
        else
            decode_PK_oil(Pub_i, pk, i);
        VOX_GenPK_vin(Pub_i, pk, i);
        if (Fqc_EvalMQSystem(Pub_i, Sig) != M[i])
            return INVALID_SIG;
    }
