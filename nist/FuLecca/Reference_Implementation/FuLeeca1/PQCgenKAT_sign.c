...
int main(void) {
 ...
 printf("\nMessage: %s\n",showhex(m,mlen));
  printf("\nAlice Public key (64th of key): %s\n\n",showhex(pk,CRYPTO_PUBLICKEYBYTES/64));
  printf("Alice Secret key (64th of key): %s\n\n",showhex(sk,CRYPTO_SECRETKEYBYTES/64 ));

  printf("Signature (64th of signature): %s\n\n",showhex(sm,CRYPTO_BYTES/64));
