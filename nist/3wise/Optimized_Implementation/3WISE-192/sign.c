...
int crypto_sign(unsigned char *sm, unsigned long long *smlen,
    ...
    unsigned char hash[SHA256_DIGEST_LENGTH]={0}, hash_hex[2*SHA192_DIGEST_LENGTH+1]={0};
    sha256(m, hash, mlen);

    for (int i = 0; i < SHA192_DIGEST_LENGTH; i++) //truncate to 192 bit (24 byte)
...
int crypto_sign_open(unsigned char *m, unsigned long long *mlen,
    ...
    *mlen = smlen - CRYPTO_BYTES;
    ...
    for (int i = 0; i < SHA192_DIGEST_LENGTH; i++)
        sprintf((unsigned char*) &hash_hex[i*2],"%02x", (unsigned char) hash[i]);
