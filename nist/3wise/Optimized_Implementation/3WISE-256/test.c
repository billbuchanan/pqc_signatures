...
double mean(double *arr) {
    double sum = 0;
    for (int i = 0; i < NUMBER_OF_KEYPAIRS*SIGNATURES_PER_KEYPAIR; i++) {
        sum += arr[i];
    }
    return (double)sum / (double)NUMBER_OF_KEYPAIRS*SIGNATURES_PER_KEYPAIR;
}
...
double median(double *arr) {
    qsort(arr, NUMBER_OF_KEYPAIRS*SIGNATURES_PER_KEYPAIR, sizeof(double), cmpfunc);
    
    if (NUMBER_OF_KEYPAIRS*SIGNATURES_PER_KEYPAIR % 2 == 1) {
        return (double)arr[NUMBER_OF_KEYPAIRS*SIGNATURES_PER_KEYPAIR / 2];
    }
    
    return (double)(arr[(NUMBER_OF_KEYPAIRS*SIGNATURES_PER_KEYPAIR - 1) / 2] + arr[NUMBER_OF_KEYPAIRS*SIGNATURES_PER_KEYPAIR / 2]) / 2;
}
