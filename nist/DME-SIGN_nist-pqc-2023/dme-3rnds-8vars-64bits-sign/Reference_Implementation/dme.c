...
void generate_skey(struct skey_t *sk)
{
    ...
    sk->c[5] &= 0x7f;
    sk->c[6] &= 0x7f;
    sk->c[1] = (sk->a[0] + sk->b[0] + sk->c[0] - sk->a[1] - sk->b[2]) & 0x7f;
    sk->c[7] = (sk->a[3] + sk->b[4] + sk->c[6] - sk->a[4] - sk->b[5]) & 0x7f;
    sk->c[4] = (sk->c[2] + sk->c[5] - sk->c[3] + delta) & 0x7f;
}
...
const unsigned char *skey_parse(struct skey_t *sk, const unsigned char *s)
{
    ...
    if ((sk->c[3] = *s++) >= 128) return NULL;
    if ((sk->c[5] = *s++) >= 128) return NULL;
...
void apply_expn_inv_1(fq_elem *x, const unsigned char *a, const fq_elem *y)
{
    ...
    fq2_inv(inv1, &y[0]);
    fq2_pow_2_k(&x[0], &y[0], -a[0] & 0x7f);
    fq2_pow_2_k(tmp1,  inv1, (a[1]-a[0]-a[2]) & 0x7f);
    fq2_pow_2_k(tmp2,  &y[2], -a[2] & 0x7f);
    fq2_mul(&x[2], tmp1, tmp2);
    fq2_inv(inv2, &y[4]);
    fq2_pow_2_k(&x[4], &y[4], -a[3] & 0x7f);
    fq2_pow_2_k(tmp1,  inv2, (a[4]-a[3]-a[5]) & 0x7f);
    fq2_pow_2_k(tmp2,  &y[6], -a[5] & 0x7f);
    fq2_mul(&x[6], tmp1, tmp2);
}
...
void apply_expn_inv_2(fq_elem *x, const unsigned char *b, const fq_elem *y)
{
    ...
    fq2_pow_2_k(tmp2, inv1, (b[1]-b[0]-b[5]) & 0x7f);
    fq2_mul(&x[0], tmp1, tmp2);
    fq2_pow_2_k(&x[2], &y[2], -b[2] & 0x7f);
    fq2_inv(inv2, &y[2]);
    fq2_pow_2_k(tmp1, &y[4], -b[4] & 0x7f);
    fq2_pow_2_k(tmp2, inv2, (b[3]-b[4]-b[2]) & 0x7f);
    fq2_mul(&x[4], tmp1, tmp2);
    fq2_pow_2_k(&x[6], &y[6], -b[5] & 0x7f);
}
...
void apply_expn_inv_3(fq_elem *x, const unsigned char *c, const fq_elem *y)
{
    ...
    fq2_inv(inv1, &y[2]);
    fq2_inv(inv2, &y[4]);
    fq2_pow_2_k(tmp1, inv2,  (c[1]-c[0]-c[4]+delta) & 0x7f);
    fq2_pow_2_k(tmp2, &y[2], (c[1]-c[0]-c[2]) & 0x7f);
    fq2_mul(tmp1, tmp1, tmp2);
    fq2_pow(tmp1, tmp1, fq2_inv_delta);
    fq2_pow_2_k(tmp2, &y[0], -c[0] & 0x7f);
    fq2_mul(&x[0], tmp1, tmp2);
    fq2_pow_2_k(tmp1, inv1,  -c[2] & 0x7f);
    fq2_pow_2_k(tmp2, &y[4], (c[3]-c[2]-c[5]) & 0x7f);
    fq2_mul(tmp1, tmp1, tmp2);
    fq2_pow(&x[2], tmp1, fq2_inv_delta);
    fq2_pow_2_k(tmp1, &y[4], (c[7]-c[6]-c[5]) & 0x7f);
    fq2_pow_2_k(tmp2, inv1,  (c[7]-c[6]-c[3]+delta) & 0x7f);
    fq2_mul(tmp1, tmp1, tmp2);
    fq2_pow(tmp1, tmp1, fq2_inv_delta);
    fq2_pow_2_k(tmp2, &y[6], -c[6] & 0x7f);
    fq2_mul(&x[4], tmp1, tmp2);
    fq2_pow_2_k(tmp1, &y[2], (delta-c[3]) & 0x7f);
    fq2_pow_2_k(tmp2, inv2, -c[5] & 0x7f);
    fq2_mul(tmp1, tmp1, tmp2);
...
void generate_pkey(struct pkey_t *pk, struct skey_t *sk)
{
    ...
    /* x1, x2 */
    pk->f[0]  = (sk->a[0] + sk->b[0] + sk->c[0]) & 0x3f;
    pk->f[1]  = (sk->a[1] + sk->b[2] + sk->c[2]) & 0x3f;
    pk->f[2]  = (sk->a[1] + sk->b[2] + sk->c[4]) & 0x3f;
    pk->f[3]  = (sk->a[1] + sk->b[3] + sk->c[6]) & 0x3f;
    /* x3, x4 */
    pk->f[4]  = (sk->a[2] + sk->a[0] + sk->b[0] - sk->a[1] + sk->c[0]) & 0x3f;
    pk->f[5]  = (sk->a[2] + sk->b[2] + sk->c[2]) & 0x3f;
    pk->f[6]  = (sk->a[2] + sk->b[2] + sk->c[4]) & 0x3f;
    pk->f[7]  = (sk->a[2] + sk->b[3] + sk->c[6]) & 0x3f;
    /* x5, x6 */
    pk->f[8]  = (sk->a[4] + sk->b[1] + sk->c[0]) & 0x3f;
    pk->f[9]  = (sk->a[4] + sk->b[5] + sk->c[3]) & 0x3f;
    pk->f[10] = (sk->a[4] + sk->b[5] + sk->c[5]) & 0x3f;
    pk->f[11] = (sk->a[3] + sk->b[4] + sk->c[6]) & 0x3f;
    /* x7, x8 */
    pk->f[12] = (sk->a[5] + sk->b[1] + sk->c[0]) & 0x3f;
    pk->f[13] = (sk->a[5] + sk->b[5] + sk->c[3]) & 0x3f;
    pk->f[14] = (sk->a[5] + sk->b[5] + sk->c[5]) & 0x3f;
    pk->f[15] = (sk->a[5] + sk->a[3] + sk->b[4] - sk->a[4] + sk->c[6]) & 0x3f;
}
...
const unsigned char *pkey_parse(struct pkey_t *pk, const unsigned char *s)
{
    ...
    pk->f[2]  = (pk->f[1]  + pk->f[10] - pk->f[9] + delta) & 0x3f;
    pk->f[4]  = (pk->f[0]  + pk->f[5]  - pk->f[1]) & 0x3f;
    pk->f[6]  = (pk->f[5]  + pk->f[2]  - pk->f[1]) & 0x3f;
    pk->f[7]  = (pk->f[5]  + pk->f[3]  - pk->f[1]) & 0x3f;
    pk->f[13] = (pk->f[12] + pk->f[9]  - pk->f[8]) & 0x3f;
    pk->f[14] = (pk->f[12] + pk->f[10] - pk->f[8]) & 0x3f;
    pk->f[15] = (pk->f[11] + pk->f[12] - pk->f[8]) & 0x3f;
    return s;
}
