...
/* see inner.h */
void shake256_init(inner_shake256_context *sc)
{
	sc->dptr = 0;
...
/* see inner.h */
void shake256_inject(inner_shake256_context *sc, const uint8_t *in, size_t len)
{
	size_t dptr;
...
/* see falcon.h */
void shake256_flip(inner_shake256_context *sc)
{
	unsigned v;
	v = sc->dptr;
...
/* see falcon.h */
void shake256_extract(inner_shake256_context *sc, uint8_t *out, size_t len)
{
	size_t dptr;
...
{
	shake256_extract(src, p->state.d, 256);

	prng_refill(p);
}
void get_seed(size_t len, prng *rng, uint8_t seed[32])
{
	// uint8_t *seed = (uint8_t *)malloc(len * sizeof(uint8_t));
	if (seed == NULL)
	{
		uint8_t *seed = (uint8_t *)malloc(len * sizeof(uint8_t));
		sysrandombytes(seed, len);

		inner_shake256_context ctx;
		shake256_init(&ctx);
		shake256_inject(&ctx, seed, len);
		prng_init(rng, &ctx);
...
{
	prng rng;
	uint8_t seed[32] = {0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7};
	get_seed(32, &rng, seed);
	uint8_t ra[136];
	for (int i = 0; i < 10; i++)
	{
		prng_get_bytes(&rng, ra, 8);
	}
}
