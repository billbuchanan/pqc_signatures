...
void samplep(int32_t * p, const uint8_t  *sk,double * L_22, double * L_32, double * L_33, uint8_t seed[32]){ 
	...
	for(int i = 0; i < PARAM_M; i++) y2[i] = y2[i] * factor1;
	...
	for (int i = 0; i < PARAM_N/4;i++){
		int32_t S[4*PARAM_M];
		unpack_mat_r(S, sk+2 * PARAM_M * PARAM_M / 8+PARAM_M*i, PARAM_M);
		real_mat_mul_int32(temp1+4*i, S, y2, 4, PARAM_M,1);
	}
	...
	for (int i = 0; i < PARAM_M / 4;i++){
		int32_t E[4*PARAM_M];
		unpack_mat_r(E, sk+PARAM_M*i, PARAM_M);
		real_mat_mul_int32(temp0+4*i, E, y2, 4, PARAM_M,1);
	}
	real_mat_add(c0, c0, temp0, PARAM_M, 1);
