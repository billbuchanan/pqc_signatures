...
void sign_mira_128_compute_shares(sign_mira_128_shares_t *shares, const gf16_mat x, const gfqm_vec A, const uint8_t *theta_i, const uint8_t *salt) {
  ...
  for(size_t i = 0; i < SIGN_MIRA_128_PARAM_N_MPC; i+=4) {
    ...
    seedexpander_shake_x4_init(&seedexpander_x4, seed_x4, SIGN_MIRA_128_SECURITY_BYTES, salt_x4, 2 * SIGN_MIRA_128_SECURITY_BYTES);
    seedexpander_shake_x4_get_bytes(&seedexpander_x4, random_x4, (2 * SIGN_MIRA_128_VEC_R_BYTES + SIGN_MIRA_128_PARAM_M_BYTES + SIGN_MIRA_128_VEC_K_BYTES));
...
void sign_mira_128_commit_to_shares(uint8_t *commits, uint8_t e, const uint8_t *salt, const sign_mira_128_shares_t *shares, const uint8_t *theta_i) {
  ...
  for(size_t i = 0; i < SIGN_MIRA_128_PARAM_N_MPC; i+=4) {
    ...
    uint8_t *commit_x4[] = {&commits[2 * SIGN_MIRA_128_SECURITY_BYTES * i], &commits[2 * SIGN_MIRA_128_SECURITY_BYTES * (i + 1)],
                            &commits[2 * SIGN_MIRA_128_SECURITY_BYTES * (i + 2)], &commits[2 * SIGN_MIRA_128_SECURITY_BYTES * (i + 3)]};

    hash_sha3_x4_ctx ctx;
    hash_sha3_x4_init(&ctx);
    hash_sha3_x4_absorb(&ctx, (const uint8_t **) domain_separator_x4, sizeof(uint8_t));
    hash_sha3_x4_absorb(&ctx, (const uint8_t **) salt_x4, 2 * SIGN_MIRA_128_SECURITY_BYTES);
    hash_sha3_x4_absorb(&ctx, (const uint8_t **) e_x4, sizeof(uint8_t));
    hash_sha3_x4_absorb(&ctx, (const uint8_t **) index_x4, sizeof(uint8_t));
    hash_sha3_x4_absorb(&ctx, (const uint8_t **) seed_x4, SIGN_MIRA_128_SECURITY_BYTES);
