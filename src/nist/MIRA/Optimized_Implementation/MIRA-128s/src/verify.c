...
/**
 * \fn int sign_mira_128_verify(const uint8_t* message, size_t message_size, const uint8_t* signature, size_t signature_size, const uint8_t* pk)
 * \brief Verify of the MIRA_128_SIGN scheme
 *
...
int sign_mira_128_verify(const uint8_t* signature, size_t signature_size, const uint8_t* message, size_t message_size, const uint8_t* pk) {
  ...
  if(signature_size != SIGN_MIRA_128_SIGNATURE_BYTES) { return EXIT_FAILURE; }

  // Setup variables related to randomness, salt, hash and challenges
  uint8_t salt[2 * SIGN_MIRA_128_SECURITY_BYTES] = {0};
  memcpy(salt, &signature[0], 2 * SIGN_MIRA_128_SECURITY_BYTES);

  uint8_t domain_separator;
  uint8_t h1[2 * SIGN_MIRA_128_SECURITY_BYTES] = {0};
  uint8_t h2[2 * SIGN_MIRA_128_SECURITY_BYTES] = {0};
  uint8_t h1_[2 * SIGN_MIRA_128_SECURITY_BYTES] = {0};
  uint8_t h2_[2 * SIGN_MIRA_128_SECURITY_BYTES] = {0};
  ...
  gf16_mat x[SIGN_MIRA_128_PARAM_TAU];
  gfqm_vec beta[SIGN_MIRA_128_PARAM_TAU];
  gfqm_vec a[SIGN_MIRA_128_PARAM_TAU];
  gfqm c[SIGN_MIRA_128_PARAM_TAU];

  for(size_t e = 0; e < SIGN_MIRA_128_PARAM_TAU; e++) {
      gf16_mat_init(&(x[e]), 1, SIGN_MIRA_128_PARAM_K);
      gfqm_vec_init(&(beta[e]), SIGN_MIRA_128_PARAM_R);
      gfqm_vec_init(&(a[e]), SIGN_MIRA_128_PARAM_R);
      gfqm_set_zero(c[e]);
  }

  uint8_t cmt[SIGN_MIRA_128_PARAM_COMMITMENT_BYTES * SIGN_MIRA_128_PARAM_TAU] = {0};
  ...
  // Parse public key
  sign_mira_128_public_key_from_string(M0, Mi, pk);
  ...
  memcpy(h1_, &signature[2 * SIGN_MIRA_128_SECURITY_BYTES], 2 * SIGN_MIRA_128_SECURITY_BYTES);
  for(size_t e = 0; e < SIGN_MIRA_128_PARAM_TAU; e++) { sign_mira_128_init_challenge1(&(challenge1[e])); }
  sign_mira_128_compute_challenge1(challenge1, h1_, salt);

  // Recompute second challenge
  memcpy(h2_, &signature[4 * SIGN_MIRA_128_SECURITY_BYTES], 2 * SIGN_MIRA_128_SECURITY_BYTES);
  hash_shake((uint8_t *) challenge2, SIGN_MIRA_128_PARAM_TAU, h2_, 2 * SIGN_MIRA_128_SECURITY_BYTES);

  // ------------------------------------------- If the challenge2 is (RYDE_PARAM_N_MPC - 1) we verify the STATE is zero
  uint8_t zero_state[SIGN_MIRA_128_PARAM_STATE_BYTES] = {0};
  for(size_t e = 0; e < SIGN_MIRA_128_PARAM_TAU; e++) {
    ...
    if (challenge2[e] == (uint8_t)(SIGN_MIRA_128_PARAM_N_MPC - 1)) {
      if (memcmp(zero_state,  &signature[index + SIGN_MIRA_128_VEC_R_BYTES], SIGN_MIRA_128_PARAM_STATE_BYTES) != 0) {
  ...
  for(size_t e = 0; e < SIGN_MIRA_128_PARAM_TAU; e++) {
    uint8_t theta_i[SIGN_MIRA_128_PARAM_TREE_LEAF_BYTES] = {0};
    challenge2[e] &= (uint8_t)(SIGN_MIRA_128_PARAM_N_MPC - 1);

    // Recompute commitments com_i
    sign_mira_128_recompute_commitments(&cmt[SIGN_MIRA_128_PARAM_COMMITMENT_BYTES * e], theta_i, e, salt,
                                         &signature[6 * SIGN_MIRA_128_SECURITY_BYTES + (SIGN_MIRA_128_PARAM_RESPONSE_BYTES * e)], challenge2[e]);

    hash_sha3_absorb(&ctx_h1, &cmt[SIGN_MIRA_128_PARAM_COMMITMENT_BYTES * e], SIGN_MIRA_128_PARAM_COMMITMENT_BYTES);

    // Recompute (leaf party) shares
    sign_mira_128_recompute_shares(&shares, theta_i, &signature[6 * SIGN_MIRA_128_SECURITY_BYTES + (SIGN_MIRA_128_PARAM_RESPONSE_BYTES * e) + SIGN_MIRA_128_PARAM_TREE_PATH_BYTES + 2 * SIGN_MIRA_128_SECURITY_BYTES + SIGN_MIRA_128_VEC_R_BYTES], challenge2[e], salt);

    memset(theta_i, 0, SIGN_MIRA_128_PARAM_TREE_LEAF_BYTES);

    // Recompute (main party) shares
    sign_mira_128_init_main_shares(&(main_shares[e]));
    sign_mira_128_recompute_main_shares(&(main_shares[e]), &shares, challenge2[e]);

    // We additional vectors required to recover the plain alpha
    sign_mira_128_recompute_additional_main_share(x[e], beta[e], a[e], c[e], &shares, challenge2[e]);
  }
  sign_mira_128_clear_shares(&shares);
  ...
  for(size_t e = 0; e < SIGN_MIRA_128_PARAM_TAU; e++) {
    ...
    sign_mira_128_add_alpha_ch2(&(response1[e]), &signature[6 * SIGN_MIRA_128_SECURITY_BYTES + (SIGN_MIRA_128_PARAM_RESPONSE_BYTES * e) +
                                                             SIGN_MIRA_128_PARAM_TREE_PATH_BYTES + 2 * SIGN_MIRA_128_SECURITY_BYTES ], challenge2[e]);

    sign_mira_128_recompute_response1(&(response1[e]), &(main_shares[e]), &(challenge1[e]), x[e], beta[e], a[e], c[e], M0, Mi, challenge2[e]);
  ...
  hash_sha3_finalize(h2, &ctx_h2);
    ...
    printf("\nh1 : "); for(size_t i = 0; i < (2 * SIGN_MIRA_128_SECURITY_BYTES); i++) { printf("%02x", h1[i]); }
    printf("\nh1_: "); for(size_t i = 0; i < (2 * SIGN_MIRA_128_SECURITY_BYTES); i++) { printf("%02x", h1_[i]); }
    printf("\nh2 : "); for(size_t i = 0; i < (2 * SIGN_MIRA_128_SECURITY_BYTES); i++) { printf("%02x", h2[i]); }
    printf("\nh2_: "); for(size_t i = 0; i < (2 * SIGN_MIRA_128_SECURITY_BYTES); i++) { printf("%02x", h2_[i]); }
    printf("\n");
  #endif

  // -------------------------------------------------------------------------------------------------------- Clear Data

  memset(cmt, 0, SIGN_MIRA_128_PARAM_COMMITMENT_BYTES * SIGN_MIRA_128_PARAM_TAU);
  ...
  if (memcmp(h1, h1_, 2*SIGN_MIRA_128_SECURITY_BYTES) != 0 || memcmp(h2, h2_, 2*SIGN_MIRA_128_SECURITY_BYTES) != 0) {
    memset(h1, 0, 2 * SIGN_MIRA_128_SECURITY_BYTES);
    memset(h2, 0, 2 * SIGN_MIRA_128_SECURITY_BYTES);
    memset(h1_, 0, 2 * SIGN_MIRA_128_SECURITY_BYTES);
    memset(h2_, 0, 2 * SIGN_MIRA_128_SECURITY_BYTES);
