...
void wots_gen_leafx1(unsigned char *dest,
    ...
    unsigned int i, k;
    unsigned char pk_buffer[ ASGN_WOTS_BYTES ];
    ...
    for (i = 0, buffer = pk_buffer; i < ASGN_WOTS_LEN; i++, buffer += ASGN_N) {
        ...
        set_type(leaf_addr, ASGN_ADDR_TYPE_WOTSPRF);

        prf_addr(buffer, ctx, leaf_addr);

        set_type(leaf_addr, ASGN_ADDR_TYPE_WOTS);
        ...
        for (k=0;; k++) {
            ...
            if (k == wots_k) {
                memcpy( info->wots_sig + i * ASGN_N, buffer, ASGN_N );
            }

            /* Check if we hit the top of the chain */
            if (k == ASGN_WOTS_W - 1) break;
    ...
    thash(dest, pk_buffer, ASGN_WOTS_LEN, ctx, pk_addr);
}
