...
size_t pack_u(unsigned char *r, const int64_t u[N_MAX], const size_t n)
{
    ...
    for (i = 0; i < n >> 3; i++)
    {
        ...
        r[1]=u[(i<<3)]>>8;
        r[2]=u[(i<<3)]>>16;
        r[3]=(u[(i<<3)]>>24)|(u[(i<<3)+1]<<6);
        r[4]=u[(i<<3)+1]>>2;
        r[5]=u[(i<<3)+1]>>10;
        r[6]=u[(i<<3)+1]>>18;
        r[7]=(u[(i<<3)+1]>>26)|(u[(i<<3)+2]<<4);
        r[8]=u[(i<<3)+2]>>4;
        r[9]=u[(i<<3)+2]>>12;
        r[10]=u[(i<<3)+2]>>20;
        r[11]=(u[(i<<3)+2]>>28)|(u[(i<<3)+3]<<2);
        r[12]=u[(i<<3)+3]>>6;
        r[13]=u[(i<<3)+3]>>14;
        r[14]=u[(i<<3)+3]>>22;
        r[15]=u[(i<<3)+4];
        r[16]=u[(i<<3)+4]>>8;
        r[17]=u[(i<<3)+4]>>16;
        r[18]=(u[(i<<3)+4]>>24)|(u[(i<<3)+5]<<6);
        r[19]=u[(i<<3)+5]>>2;
        r[20]=u[(i<<3)+5]>>10;
        r[21]=u[(i<<3)+5]>>18;
        r[22]=(u[(i<<3)+5]>>26)|(u[(i<<3)+6]<<4);
        r[23]=u[(i<<3)+6]>>4;
        r[24]=u[(i<<3)+6]>>12;
        r[25]=u[(i<<3)+6]>>20;
        r[26]=(u[(i<<3)+6]>>28)|(u[(i<<3)+7]<<2);
        r[27]=u[(i<<3)+7]>>6;
        r[28]=u[(i<<3)+7]>>14;
        r[29]=u[(i<<3)+7]>>22;
