...
/**
 * \fn int ryde_256s_keygen(uint8_t* pk, uint8_t* sk)
 ...
 */
int ryde_256s_keygen(uint8_t* pk, uint8_t* sk) {
  #ifdef VERBOSE
    printf("\n\n\n### KEYGEN ###");
  #endif
  ...
  if (randombytes(sk_seed, RYDE_256S_SECURITY_BYTES) != EXIT_SUCCESS) {
    memset(sk_seed, 0, RYDE_256S_SECURITY_BYTES);
    return EXIT_FAILURE;
  }
  if (randombytes(pk_seed, RYDE_256S_SECURITY_BYTES) != EXIT_SUCCESS) {
    memset(sk_seed, 0, RYDE_256S_SECURITY_BYTES);
    memset(pk_seed, 0, RYDE_256S_SECURITY_BYTES);
    return EXIT_FAILURE;
  }
  ...
  ryde_256s_public_key_to_string(pk, pk_seed, y);
  ryde_256s_secret_key_to_string(sk, sk_seed, pk_seed);

  #ifdef VERBOSE
    printf("\n\nsk_seed: "); for(int i = 0 ; i < RYDE_256S_SECURITY_BYTES ; ++i) printf("%02x", sk_seed[i]);
    printf("\npk_seed: ");   for(int i = 0 ; i < RYDE_256S_SECURITY_BYTES ; ++i) printf("%02x", pk_seed[i]);
    printf("\nsk: "); for(int i = 0 ; i < RYDE_256S_SECRET_KEY_BYTES ; ++i) printf("%02x", sk[i]);
    printf("\npk: "); for(int i = 0 ; i < RYDE_256S_PUBLIC_KEY_BYTES ; ++i) printf("%02x", pk[i]);

    printf("\nx:");
    size_t length = ((RYDE_256S_PARAM_N - RYDE_256S_PARAM_K) * RYDE_256S_PARAM_M + 7 ) / 8;
    uint8_t x1_string[length];
    rbc_43_vec_to_string(x1_string, x1, RYDE_256S_PARAM_N - RYDE_256S_PARAM_K);
    printf("\n    - x1   : "); for(size_t i = 0 ; i < length ; i++) { printf("%02x", x1_string[i]); }
    memset(x1_string, 0, length);
    uint8_t x2_string[RYDE_256S_VEC_K_BYTES] = {0};
    rbc_43_vec_to_string(x2_string, x2, RYDE_256S_PARAM_K);
    printf("\n    - x2   : "); for(size_t i = 0 ; i < RYDE_256S_VEC_K_BYTES ; i++) { printf("%02x", x2_string[i]); }
    memset(x2_string, 0, RYDE_256S_VEC_K_BYTES);

    length = ((RYDE_256S_PARAM_N - RYDE_256S_PARAM_K) * RYDE_256S_PARAM_K * RYDE_256S_PARAM_M + 7 ) / 8;
    uint8_t H_string[length];
    rbc_43_mat_to_string(H_string, H, RYDE_256S_PARAM_N - RYDE_256S_PARAM_K, RYDE_256S_PARAM_K);
    printf("\nH: "); for(size_t i = 0 ; i < length ; i++) { printf("%02x", H_string[i]); }
    memset(H_string, 0, length);

    length = ((RYDE_256S_PARAM_N - RYDE_256S_PARAM_K) * RYDE_256S_PARAM_M + 7 ) / 8;
    uint8_t y_string[length];
    rbc_43_vec_to_string(y_string, y, RYDE_256S_PARAM_N - RYDE_256S_PARAM_K);
