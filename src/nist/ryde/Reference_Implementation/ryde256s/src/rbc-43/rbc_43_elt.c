...
void rbc_43_elt_ur_mul(rbc_43_elt_ur o, const rbc_43_elt e1, const rbc_43_elt e2) {
  uint64_t shifts[64][RBC_43_ELT_SIZE + 1];
  rbc_43_elt_set(shifts[0], e2);
  shifts[0][RBC_43_ELT_SIZE] = 0;
  
  for(uint8_t shift=1 ; shift<64 ; shift++) {
    shifts[shift][0] = shifts[shift-1][0] << 1;
    for(uint8_t i=1 ; i<RBC_43_ELT_SIZE + 1 ; i++) {
      shifts[shift][i] = (shifts[shift-1][i] << 1) | (shifts[shift-1][i-1] >> 63);
    }
  }
  
  rbc_43_elt_ur_set_zero(o);
  for(uint8_t i=0 ; i<RBC_43_FIELD_M ; i++) {
    uint8_t shift = i % 64;
    uint8_t offset = i / 64;
    uint64_t multiplier = (e1[offset] >> shift) & 0x1;
    for(uint8_t j=0 ; j<RBC_43_ELT_SIZE + 1 ; j++) {
