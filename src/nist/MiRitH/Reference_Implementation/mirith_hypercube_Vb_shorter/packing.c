...
void pack_secret_key(uint8_t *sk,
    const ff_t M0[matrix_bytes_size(PAR_M, PAR_N)], const seed_t seed_pk,
...
void pack_signature(
    ...
    ff_t K_last_shr[TAU][matrix_bytes_size(PAR_R, PAR_N - PAR_R)],
    ff_t C_last_shr[TAU][matrix_bytes_size(PAR_S, PAR_N - PAR_R)],
    ff_t S_rnd_star[TAU][matrix_bytes_size(PAR_S, PAR_R)])
{
     ...
     *
     *      for l in {0, 1, ..., TAU}
     *          pack(com_star[l][i_star[l]])
     *          pack(packed_tree[l])
     *
     *      for l in {0, 1, ..., TAU}
     *          if i_star[l] != N_PARTIES - 1
     *              pack(a_last_shr[l][N_PARTIES - 1])
     *              pack(K_last_shr[l][N_PARTIES - 1])
     *              pack(C_last_shr[l][N_PARTIES - 1])
     *          pack(S_rnd_star[l]])
     */
    ...
    {
        /* Pack 'com[l][i_star[l]]'. */
        memcpy(ptr, com_star[l], HASH_SIZE); //
        ptr += HASH_SIZE;
    ...
    for (l = 0; l < TAU; l++)
    {
        if (i_star[l] != N_PARTIES_ROUND - 1)
        {
            ...
            matrix_pack(&ptr, &bo, a_last_shr[l], PAR_K, 1);
            matrix_pack(&ptr, &bo, K_last_shr[l], PAR_R, PAR_N - PAR_R);
            matrix_pack(&ptr, &bo, C_last_shr[l], PAR_S, PAR_N - PAR_R);
        }

        /* Pack 'S_rnd_star[l]'. */
        matrix_pack(&ptr, &bo, S_rnd_star[l], PAR_S, PAR_R);
    }
