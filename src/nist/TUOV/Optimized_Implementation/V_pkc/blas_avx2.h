...
static inline
unsigned char xor128_bytes(__m128i out) {
    unsigned char _table[16] __attribute__((aligned(16)));

    _mm_store_si128((__m128i *)_table, out);
...
static inline
unsigned char xor256_bytes(__m256i out) {
    unsigned char _table[32] __attribute__((aligned(32)));

    _mm256_store_si256((__m256i *)_table, out);
...
static inline
void linearmap_8x8_accu_mul_sse( uint8_t *accu_c, const uint8_t *a, const __m128i *multabs, __m128i mask, unsigned _num_byte ) {
    ...
    while ( n_16-- ) {
        ...
        __m128i tab_l = multabs[0];
        __m128i tab_h = multabs[1];
        multabs += 2;

        __m128i inp = _mm_loadu_si128( (__m128i *)(a) );

        out ^= linear_transform_8x8_128b( tab_l, tab_h, inp, mask );
        a += 16;
    }
    *accu_c ^= xor128_bytes(out);
    ...
    if ( rem ) {
        ...
        __m128i out = _mm_setzero_si128();
        __m128i r0 = linear_transform_8x8_128b( tab_l, tab_h, inp, mask );
        r0 ^= out;
        *accu_c ^= xor128_bytes(out);
    }
}
...
static inline
void _gf16v_mul_add_avx2(uint8_t *accu_c, const unsigned char *a, const unsigned char *multab_b, unsigned _num_byte) {
    ...
    for (unsigned i = 0; i < n_32; i++ ) {
        ...
        __m256i ml = _mm256_load_si256( (__m256i *) (multabs) );
        __m256i mh = _mm256_slli_epi16( ml, 4 );
        multabs ++;

        __m256i inp = _mm256_loadu_si256( (__m256i *)a );
        
        out = out ^ linear_transform_8x8_256b( ml, mh, inp, mask_f );
        a += 32;
    }
    *accu_c = xor256_bytes(out);

    if ( rem ) {
        linearmap_8x8_accu_mul_sse( accu_c, a, (const __m128i *)multabs, _mm256_castsi256_si128(mask_f), rem );
    }
}

static inline
void _gf256v_mul_add_avx2(unsigned char *accu_c, const unsigned char *a, const unsigned char *multab_b, unsigned _num_byte) {
