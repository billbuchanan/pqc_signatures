...
int crypto_sign_keypair(unsigned char* pk, unsigned char* sk) {
  int ret = faest_192s_keygen(pk, SK_KEY(sk));
  if (!ret) {
...
int crypto_sign(unsigned char* sm, unsigned long long* smlen, const unsigned char* m,
  ...
  if (!faest_192s_owf(SK_KEY(sk), PK_INPUT(pk), PK_OUTPUT(pk))) {
    // invalid key
    return -1;
  }
  ...
  *smlen = mlen + FAEST_192S_SIGNATURE_SIZE;
  ...
  return faest_192s_sign(SK_KEY(sk), pk, sm, mlen, NULL, 0, sm + mlen, &signature_len);
#else
  uint8_t rho[FAEST_192S_LAMBDA / 8];
  rand_bytes(rho, sizeof(rho));
  return faest_192s_sign(SK_KEY(sk), pk, sm, mlen, rho, sizeof(rho), sm + mlen, &signature_len);
#endif
}
