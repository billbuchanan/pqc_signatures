                ...
                signature_t* sig)
{
  ...
  const size_t GF_byte_size     = sizeof(GF);
  ...
  GF* sbox_outputs = malloc((L + 1) * GF_byte_size);
  ...
  GF* repetition_shared_x     = malloc(tau * N * (L + 1) * GF_byte_size);
  GF* repetition_shared_z     = malloc(tau * N * (L + 1) * GF_byte_size);
  GF* repetition_shared_dot_a = malloc(tau * N * GF_byte_size);
  GF* repetition_shared_dot_c = malloc(tau * N * GF_byte_size);
  ...
  for (size_t repetition = 0; repetition < tau; repetition++)
  {
    ...
    memset(proof->z_delta, 0, L * GF_byte_size);
    for (size_t party = 0; party < N; party++)
    {
      GF* shared_z =
  ...
  //////////////////////////////////////////////////////////////////////////////
  GF*  repetition_alpha_shares = calloc(tau * N * GF_byte_size, 1);
  ...
  for (size_t repetition = 0; repetition < tau; repetition++)
  {
    ...
    v_shares[repetition] = malloc(N * GF_byte_size);
    for (size_t party = 0; party < N; party++)
    {
                  ...
                  const uint8_t* message, const size_t message_len)
{
  ...
  for (size_t repetition = 0; repetition < tau; repetition++)
  {
    ...
    v_shares[repetition] = calloc(N * GF_byte_size, 1);
    GF pt_share = {0,};
    for (size_t party = 0; party < N; party++)
    {
      if (party != missing_party)
