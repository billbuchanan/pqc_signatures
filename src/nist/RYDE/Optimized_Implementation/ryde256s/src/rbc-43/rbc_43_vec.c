/**
 * \file rbc_43_vec.c
 * \brief Implementation of rbc_43_vec.h
 */
...
/**
 * \fn void rbc_43_vec_init(rbc_43_vec* v, uint32_t size)
 * \brief This function initiates a rbc_43_vec element.
 *
 * \param[out] v Pointer to a rbc_43_vec element
 * \param[in] size Size of the vector
 */
void rbc_43_vec_init(rbc_43_vec* v, uint32_t size) {
  *v = calloc(size, sizeof(rbc_43_elt));
...
/**
 * \fn void rbc_43_vec_clear(rbc_43_vec v)
 * \brief This function clears a rbc_43_vec element.
 *
 * \param[out] v rbc_43_vec
 */
...
/**
 * \fn void rbc_43_vec_set_zero(rbc_43_vec v, uint32_t size)
...
/**
 * \fn void rbc_43_vec_set_random(seedexpander_shake_t*, rbc_43_vec v, uint32_t size)
 ...
 * \param[in] size Size of the vector
 */
void rbc_43_vec_set_random(seedexpander_shake_t* ctx, rbc_43_vec v, uint32_t size) {
  uint32_t bytes = (RBC_43_FIELD_M % 8 == 0) ? RBC_43_FIELD_M / 8 : RBC_43_FIELD_M / 8 + 1;
  uint8_t random[size * bytes];
  uint8_t mask = (1 << RBC_43_FIELD_M % 8) - 1;

  rbc_43_vec_set_zero(v, size);
...
/**
 * \fn void rbc_43_vec_set_random_full_rank_with_one(seedexpander_shake_t* ctx, rbc_43_vec o, uint32_t size) {
...
void rbc_43_vec_set_random_full_rank_with_one(seedexpander_shake_t* ctx, rbc_43_vec o, uint32_t size) {
  int32_t rank_max = RBC_43_FIELD_M < size ? RBC_43_FIELD_M : size;
  int32_t rank = -1;

  while(rank != rank_max) {
    rbc_43_vec_set_random(ctx, o, size - 1);
    rbc_43_elt_set_one(o[size - 1]);
    rank = rbc_43_vec_get_rank(o, size);
  }
}
...
/**
 * \fn void rbc_43_vec_set_random_from_support(seedexpander_shake_t* o, rbc_43_vec o, uint32_t size, const rbc_43_vec support, uint32_t support_size)
...
void rbc_43_vec_set_random_from_support(seedexpander_shake_t* ctx, rbc_43_vec o, uint32_t size, const rbc_43_vec support, uint32_t support_size, uint8_t copy_flag) {
  ...
  if(copy_flag) {
    ...
    while(i != support_size) {
      ...
      if(position < size * (256 / size) && rbc_43_elt_is_zero(o[position % size])) {
        rbc_43_elt_set(o[position % size], support[i]);
        i++;
      }
    ...
    uint32_t l = 0;
    for(i = 0 ; i < size ; ++i) {
      if(rbc_43_elt_is_zero(o[i])) {

        for(j = 0 ; j < support_size ; ++j) {
          if(random2[k] & 0x1) {
            rbc_43_elt_add(o[i], support[j], o[i]);
          }
    ...
    for(i=0 ; i<size ; i++) {
      for(j=0 ; j<support_size ; j++) {
        if(random[k] & 0x01) {
          rbc_43_elt_add(o[i], o[i], support[j]);
        }
...
/**
 * \fn uint32_t rbc_43_vec_gauss(rbc_43_vec v, uint32_t size, rbc_43_vec *other_matrices, uint32_t nMatrices)
...
uint32_t rbc_43_vec_gauss(rbc_43_vec v, uint32_t size, uint8_t reduced_flag, rbc_43_vec *other_matrices, uint32_t nMatrices) {
  ...
  for(uint32_t p = 0 ; p < size ; p++) {
    rbc_43_elt acc;
    rbc_43_elt_set(acc, v[p]);
    for(uint32_t i=p+1 ; i<size ; i++) {
      for(uint32_t j=0 ; j<RBC_43_ELT_DATA_SIZE ; j++) {
        acc[j] |= v[i][j];
      }
    }

    int column = rbc_43_elt_get_degree(acc);
    column += (column < 0);

    dimension += (!rbc_43_elt_is_zero(acc));

    //For each line below
    for(uint32_t i=p+1 ; i < size ; i++) {
        mask = rbc_43_elt_get_coefficient(v[p], column);
        rbc_43_elt_set_mask1(tmp, zero, v[i], mask);
        rbc_43_elt_add(v[p], v[p], tmp);

        for(uint32_t k=0 ; k<nMatrices ; k++) {
          rbc_43_elt_set_mask1(tmp, zero, other_matrices[k][i], mask);
          rbc_43_elt_add(other_matrices[k][p], other_matrices[k][p], tmp);
        }
    }
    ...
    if(reduced_flag) {
      for(uint32_t i=0 ; i < p ; i++) {
        mask = rbc_43_elt_get_coefficient(v[i], column);
        rbc_43_elt_set_mask1(tmp, v[p], zero, mask);
        rbc_43_elt_add(v[i], v[i], tmp);

        for(uint32_t k=0 ; k<nMatrices ; k++) {
          rbc_43_elt_set_mask1(tmp, other_matrices[k][p], zero, mask);
          rbc_43_elt_add(other_matrices[k][i], other_matrices[k][i], tmp);
        }
      }
    }
...
/**
 * \fn uint32_t rbc_43_vec_get_rank(const rbc_43_vec v, uint32_t size)
...
uint32_t rbc_43_vec_get_rank(const rbc_43_vec v, uint32_t size) {
  ...
  rbc_43_vec_init(&copy, size);
  rbc_43_vec_set(copy, v, size);
  dimension = rbc_43_vec_gauss(copy, size, 0, NULL, 0);
  rbc_43_vec_clear(copy);
...
/**
 * \fn void rbc_43_vec_inner_product(rbc_43_elt o, const rbc_43_vec v1, const rbc_43_vec v2, uint32_t size)
...
void rbc_43_vec_inner_product(rbc_43_elt o, const rbc_43_vec v1, const rbc_43_vec v2, uint32_t size) {
  ...
  for(size_t i = 0 ; i < size ; ++i) {
    rbc_43_elt_mul(t, v1[i], v2[i]);
    rbc_43_elt_add(o, o, t);
  }
  rbc_43_elt_set_zero(t);
}
...
/**
 * \fn void rbc_43_vec_scalar_mul(rbc_43_vec o, const rbc_43_vec v, const rbc_43_elt e, uint32_t size)
 * \brief This functions multiplies a vector of finite field elements by a scalar.
 *
 * \param[out] o rbc_43_vec equal to \f$ e \times v \f$
 * \param[in] v rbc_43_vec
...
/**
 * \fn void rbc_43_vec_to_string(uint8_t* str, const rbc_43_vec v, uint32_t size)
...
void rbc_43_vec_to_string(uint8_t* str, const rbc_43_vec v, uint32_t size) {
  ...
  uint32_t str_size = ((size * RBC_43_FIELD_M) % 8 == 0) ? (size * RBC_43_FIELD_M) / 8 : (size * RBC_43_FIELD_M) / 8 + 1;
  ...
  for(size_t i = 0 ; i < size ; i++) {
    for(size_t j = 1 ; j <= bytes2 ; j++) {
      uint8_t bit = rbc_43_elt_get_coefficient(v[i], RBC_43_FIELD_M - j);
      *(str + index) |= (bit << k % 8);
      k++;
      if(k % 8 == 0) index++;
    }
...
/**
 * \fn void rbc_43_vec_from_string(rbc_43_vec v, uint32_t size, const uint8_t* str)
...
void rbc_43_vec_from_string(rbc_43_vec v, uint32_t size, const uint8_t* str) {
  ...
  for(size_t i = 0 ; i < size ; i++) {
    for(size_t j = 1 ; j <= bytes2 ; j++) {
      ...
      rbc_43_elt_set_coefficient_vartime(v[i], RBC_43_FIELD_M - j, bit);
      k++;
      if(k % 8 == 0) index++;
    }
  }
...
/**
 * \fn void rbc_43_vec_from_bytes(rbc_43_vec o, uint32_t size, const uint8_t *random)
...
void rbc_43_vec_from_bytes(rbc_43_vec o, uint32_t size, uint8_t *random) {
  uint8_t mask = (1 << RBC_43_FIELD_M % 8) - 1;
  if ((RBC_43_FIELD_M % 8) == 0) { mask = 0xff; }

  for(size_t i = 0 ; i < size ; ++i) {
    random[(i + 1) * RBC_43_ELT_UINT8_SIZE - 1] &= mask;
    memcpy(o[i], &random[i * RBC_43_ELT_UINT8_SIZE], RBC_43_ELT_UINT8_SIZE);
  }
}
...
/**
 * \fn void rbc_43_vec_to_bytes( uint8_t *o, const rbc_43_vec v, uint32_t size)
 * \brief This function sets a vector of finite field elements from random bytes.
 *
...
void rbc_43_vec_to_bytes( uint8_t *o, const rbc_43_vec v, uint32_t size) {
  for(size_t i = 0 ; i < size ; ++i) {
    memcpy(&o[i * RBC_43_ELT_UINT8_SIZE], v[i], RBC_43_ELT_UINT8_SIZE);
  }
}
...
/**
 * \fn void rbc_43_vec_print(rbc_43_vec v, uint32_t size)
 * \brief Display an rbc_43_vec element.
 *
 * \param[out] v rbc_43_vec
 * \param[in] size Size of the vector
 */
void rbc_43_vec_print(rbc_43_vec v, uint32_t size) {
  printf("[\n");
  for(size_t i = 0 ; i < size ; ++i) {
    rbc_43_elt_print(v[i]);
    printf("\n");
  }
  printf("]\n");
}
