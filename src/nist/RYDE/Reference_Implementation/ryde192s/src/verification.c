...
/**
 * \fn int ryde_192s_verify(const uint8_t* message, size_t message_size, const uint8_t* signature, size_t signature_size, const uint8_t* pk)
...
int ryde_192s_verify(const uint8_t* signature, size_t signature_size, const uint8_t* message, size_t message_size, const uint8_t* pk) {
  ...
  if(signature_size != RYDE_192S_SIGNATURE_BYTES) { return EXIT_FAILURE; }

  // Setup variables related to randomness, salt, hash and challenges
  uint8_t salt[2 * RYDE_192S_SECURITY_BYTES] = {0};
  memcpy(salt, &signature[0], 2 * RYDE_192S_SECURITY_BYTES);

  uint8_t domain_separator;
  uint8_t h1[2 * RYDE_192S_SECURITY_BYTES] = {0};
  uint8_t h2[2 * RYDE_192S_SECURITY_BYTES] = {0};
  uint8_t h1_[2 * RYDE_192S_SECURITY_BYTES] = {0};
  uint8_t h2_[2 * RYDE_192S_SECURITY_BYTES] = {0};
  ...
  rbc_37_vec x1[RYDE_192S_PARAM_TAU];
  rbc_37_vec x2[RYDE_192S_PARAM_TAU];
  rbc_37_vec a[RYDE_192S_PARAM_TAU];
  rbc_37_vec u[RYDE_192S_PARAM_TAU];
  rbc_37_elt c[RYDE_192S_PARAM_TAU];

  for(size_t e = 0; e < RYDE_192S_PARAM_TAU; e++) {
      rbc_37_vec_init(&(x1[e]), RYDE_192S_PARAM_N - RYDE_192S_PARAM_K);
      rbc_37_vec_init(&(x2[e]), RYDE_192S_PARAM_K);
      rbc_37_vec_init(&(a[e]), RYDE_192S_PARAM_W - 1);
      rbc_37_vec_init(&(u[e]), RYDE_192S_PARAM_W - 1);
      rbc_37_elt_set_zero(c[e]);
  }

  uint8_t cmt[RYDE_192S_PARAM_COMMITMENT_BYTES * RYDE_192S_PARAM_TAU] = {0};
  ...
  // Parse public key
  ryde_192s_public_key_from_string(H, y, pk);
  ...
  memcpy(h1_, &signature[2 * RYDE_192S_SECURITY_BYTES], 2 * RYDE_192S_SECURITY_BYTES);
  for(size_t e = 0; e < RYDE_192S_PARAM_TAU; e++) { ryde_192s_init_challenge1(&(challenge1[e])); }
  ryde_192s_compute_challenge1(challenge1, h1_, salt);

  // Recompute second challenge
  memcpy(h2_, &signature[4 * RYDE_192S_SECURITY_BYTES], 2 * RYDE_192S_SECURITY_BYTES);
  hash_shake((uint8_t *)challenge2, RYDE_192S_PARAM_TAU, h2_, 2 * RYDE_192S_SECURITY_BYTES);

  // ------------------------------------------- If the challenge2 is (RYDE_192S_PARAM_N_MPC - 1) we verify the STATE is zero
  uint8_t zero_state[RYDE_192S_PARAM_STATE_BYTES] = {0};
  for(size_t e = 0; e < RYDE_192S_PARAM_TAU; e++) {
    ...
    if (challenge2[e] == (uint8_t)(RYDE_192S_PARAM_N_MPC - 1)) {
        if (memcmp(zero_state,  &signature[index + RYDE_192S_VEC_W_MINUS_ONE_BYTES], RYDE_192S_PARAM_STATE_BYTES) != 0) {

          ryde_192s_clear_leaf_shares(&shares);
          for(size_t i = 0; i < RYDE_192S_PARAM_TAU; i++) {
            ryde_192s_clear_challenge1(&(challenge1[i]));
  ...
  for(size_t e = 0; e < RYDE_192S_PARAM_TAU; e++) {
    uint8_t theta_i[RYDE_192S_PARAM_TREE_LEAF_BYTES] = {0};
    challenge2[e] &= (uint8_t)(RYDE_192S_PARAM_N_MPC - 1);

    // Recompute commitments com_i
    ryde_192s_recompute_commitments(&cmt[RYDE_192S_PARAM_COMMITMENT_BYTES * e], theta_i, e, salt,
                                         &signature[6 * RYDE_192S_SECURITY_BYTES + (RYDE_192S_PARAM_RESPONSE_BYTES * e)], challenge2[e]);

    hash_sha3_absorb(&ctx_h1, &cmt[RYDE_192S_PARAM_COMMITMENT_BYTES * e], RYDE_192S_PARAM_COMMITMENT_BYTES);

    // Recompute (leaf party) shares
    ryde_192s_recompute_shares(&shares, y, H, theta_i, &signature[6 * RYDE_192S_SECURITY_BYTES + (RYDE_192S_PARAM_RESPONSE_BYTES * e) +
                                    RYDE_192S_PARAM_TREE_PATH_BYTES + 2 * RYDE_192S_SECURITY_BYTES + RYDE_192S_VEC_W_MINUS_ONE_BYTES], challenge2[e], salt);

    memset(theta_i, 0, RYDE_192S_PARAM_TREE_LEAF_BYTES);

    // Recompute (main party) shares
    ryde_192s_init_main_shares(&(main_shares[e]));
    ryde_192s_recompute_main_shares(&(main_shares[e]), &shares, challenge2[e]);

    // We additional vectors required to recover the plain alpha
    ryde_192s_recompute_additional_main_share(x1[e], x2[e], a[e], u[e], c[e], &shares, challenge2[e]);
  }
  ryde_192s_clear_leaf_shares(&shares);
  ...
  for(size_t e = 0; e < RYDE_192S_PARAM_TAU; e++) {
    ...
    ryde_192s_add_alpha_ch2(&(response1[e]), &signature[6 * RYDE_192S_SECURITY_BYTES + (RYDE_192S_PARAM_RESPONSE_BYTES * e) +
                                                             RYDE_192S_PARAM_TREE_PATH_BYTES + 2 * RYDE_192S_SECURITY_BYTES ], challenge2[e]);
    ryde_192s_recompute_response1(&(response1[e]), &(main_shares[e]), &(challenge1[e]), x1[e], x2[e], a[e], u[e], c[e], challenge2[e]);
  ...
  // -------------------------------------------------------------------------------------------------------- Clear Data
    ...
    printf("\nh1 : "); for(size_t i = 0; i < (2 * RYDE_192S_SECURITY_BYTES); i++) { printf("%02x", h1[i]); }
    printf("\nh1_: "); for(size_t i = 0; i < (2 * RYDE_192S_SECURITY_BYTES); i++) { printf("%02x", h1_[i]); }
    printf("\nh2 : "); for(size_t i = 0; i < (2 * RYDE_192S_SECURITY_BYTES); i++) { printf("%02x", h2[i]); }
    printf("\nh2_: "); for(size_t i = 0; i < (2 * RYDE_192S_SECURITY_BYTES); i++) { printf("%02x", h2_[i]); }
    printf("\n");
  #endif

  memset(cmt, 0, RYDE_192S_PARAM_COMMITMENT_BYTES * RYDE_192S_PARAM_TAU);
  ...
  if (memcmp(h1, h1_, 2*RYDE_192S_SECURITY_BYTES) != 0 || memcmp(h2, h2_, 2*RYDE_192S_SECURITY_BYTES) != 0) {
    memset(h1, 0, 2 * RYDE_192S_SECURITY_BYTES);
    memset(h2, 0, 2 * RYDE_192S_SECURITY_BYTES);
    memset(h1_, 0, 2 * RYDE_192S_SECURITY_BYTES);
    memset(h2_, 0, 2 * RYDE_192S_SECURITY_BYTES);
