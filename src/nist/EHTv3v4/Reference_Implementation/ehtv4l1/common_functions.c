...
/**
 * This function computes the composition index of two indices, based on predefined 3D matrices (GL_3_2).
 * It calculates a composition index, essentially finding the product of two 3x3 matrices, and finding the position
 * of the result in GL_3_2.
 *
 * @param i1 The index of the first matrix.
 * @param i2 The index of the second matrix.
 * @return The composition index of 'i1' and 'i2'.
 */
unsigned short composition_index(int i1, int i2)
{
    // Create a flat result array initialized to size GL_n2
    bool result_flat[GL_n2];

    int l = 0; // Initialize an iterator

    // Loop through the GL_n x GL_n (3x3) matrix
    for(int i=0; i<GL_n; i++)
    {
        for(int j=0; j<GL_n; j++)
        {
            // Initialize the current flat matrix element as false
            result_flat[l] = 0;

            // Perform a bitwise AND operation on the respective elements of the two matrices, and a bitwise XOR operation with the current flat matrix element
            for(int k=0; k<GL_n; k++)
            {
                result_flat[l] ^= GL_3_2[i1][i][k]  & GL_3_2[i2][k][j];
            }
    ...
    // Calculate the lookup index by summing up the shifted boolean values
    int lookup_index = 0;
    for(int i=0; i<GL_n2; i++)
    {
    	lookup_index += result_flat[i] << i;
	}
	
	// Return the value at the lookup index in the GL_3_2 lookup array
	return lookup_GL_3_2[lookup_index];
}
