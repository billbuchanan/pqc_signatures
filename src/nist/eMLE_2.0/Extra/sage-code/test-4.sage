...
########################################
def attack(pp, GG, h, k0Avg, k1Avg, withExtraConstraint):
	print ("attack starts ..........")
	#print (h)
	...
	#       1         eqs         xs                    ks                                 G*x small        G*x-k*p < p
	#wei = [big] + [big]*n + [1]*n + sum(([1]*n for p in pp),[]) + [big/pp[-1]]*n +[big//pp[0]]*n + [big//pp[1]]*n
	wei = [big] + [big]*n + [2^17]*n + [1]*n + [1]*n + [1]*n + [1]*n +[2^20]*n + [2^4]*n
	est =                       [0//2]*n + sum(([pp[-1]//n//2]*n for p in pp),[]) + [0]*n + [2*pp[1]//(4)]*n + [3*pp[2]//(4)]*n
	...
	print('-'*64)
	print(f'maybe s:  \x1b[32m{xx}\x1b[0m')
...
for i in range(1):
	...
	u, _, _ =sign(x1_org, x2_org, F1_org, F2_org, h1_org, h2_org, i)
	c1, c2 = mkVecC(str(i)+str(u)+str(h1_org)+str(h2_org))
	t = mul(h1_org, c1, n, pp[d-1]) + mul(h2_org, c2, n, pp[d-1]) + u
	cp1, cp2 = mkVecC(str(i)+str(h1_org)+str(h2_org))

	o = mul(G[1], c1+c2, n, pp[0]) + cp1 + cp2
	...
	for k0 in range(1):
		for k1 in range(1): 
			_, s, _ = attack(pp, GG, vector(ZZ, t),  5000, 0, withExtraConstraint) #last parameter indicating whether extra constrains are consdiered  

			r=verify(i,h1_org, h2_org, u,s)
			#observe whether t =0 
			if r:
